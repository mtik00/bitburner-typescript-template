...
export async function main(ns: NS): Promise<void> {
    ...
    while (ram <= maxRam && index < 30) {
        index += 1
        // ns.tprintf("step: %i; cost: %s; ram: %s", index, totalUpgradeCost, ns.formatRam(ram))
    ...
    if (!isFinite(totalUpgradeCost)) {
        ...
        ns.tprintf("ERROR: You don't have enough money to upgrade your servers to %s", ns.formatRam(ram * 2, 0))
    }
        ...
        ns.tprintf("can upgrade/purchase all servers with: %s (%s) for %s", ns.formatRam(ram), ns.formatNumber(ram, 0), ns.formatNumber(totalUpgradeCost))
    }
}
